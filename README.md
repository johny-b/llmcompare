...
You can use `ModelAdapter.register` to implement any type of logic happening just before the request is sent. Note that handlers are not called only just immediately before a request is sent, but also e.g. when llmcompare searches for cached results.
